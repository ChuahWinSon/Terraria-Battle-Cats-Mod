...
namespace TheBattleCats.Content.NPCs
{
	...
	[AutoloadHead]
	public class CatCapsule : ModNPC
	{
		private static readonly int[] RewardPool = new int[]
        {
            ItemID.Meowmere,
            ItemID.CopperShortsword,
            ItemID.GoldBar,
            // add whatever items you want here
            ModContent.ItemType<CatTicket>() // can even include modded items
        };
		...
		public override string GetChat()
        {
			...
			chat.Add(Language.GetTextValue("Mods.TheBattleCats.Dialogue.CatCapsule.StandardDialogue1"), 2.0);
			chat.Add(Language.GetTextValue("Mods.TheBattleCats.Dialogue.CatCapsule.StandardDialogue2"), 0.5);
		...
		public override void OnChatButtonClicked(bool firstButton, ref string shopName)
        {
            ...
            // Check if the player has a Cat Ticket
            int ticketIndex = player.FindItem(ModContent.ItemType<CatTicket>());

            if (ticketIndex == -1)
            {
                // Player doesn't have a ticket
                Main.npcChatText = "You don't have any Cat Tickets!";
                return;
            }

            // Consume one Cat Ticket
            player.inventory[ticketIndex].stack--;
            if (player.inventory[ticketIndex].stack <= 0)
                player.inventory[ticketIndex].TurnToAir();

            // Pick a random item from the pool
            int rewardItemID = RewardPool[Main.rand.Next(RewardPool.Length)];

            // Give the item to the player
            player.QuickSpawnItem(NPC.GetSource_GiftOrReward(), rewardItemID, 1);

            Main.npcChatText = "Enjoy your reward!";
			}
        }
