...
{
    public class ChronosAttack3 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                SoundEngine.PlaySound(new SoundStyle("TheBattleCats/Assets/Music/chronosAttack3"), Projectile.Center);
...
private void LaunchHomingProjectile()
{
    int projType = ModContent.ProjectileType<ChronosAttackDone>();

    float baseAngle = Projectile.rotation; // Rotation of the weapon (in radians)
    int numProjectiles = 6;
    float spreadDegrees = 180f; // total spread
    float spreadRadians = MathHelper.ToRadians(spreadDegrees);

    for (int i = 0; i < numProjectiles; i++)
    {
        // Evenly space projectiles across the spread
        float offset = MathHelper.Lerp(-spreadRadians / 2, spreadRadians / 2, i / (float)(numProjectiles - 1));
        ...
        {
            return Projectile.frame >= 10 && Projectile.frame <= 11;
        }
    }
}
